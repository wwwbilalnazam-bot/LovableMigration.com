...
const DEPLOYMENT_CONFIG: Record<string, { label: string; icon: React.ReactNode; cls: string }> = {
  supabase_cloud: {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="glass-card !p-6 border-white/5 bg-surface/30">
          <p className="text-text-muted text-[10px] font-black uppercase tracking-[0.2em] mb-1">Cloud Preference</p>
          <div className="flex items-end gap-3">
            <h3 className="text-3xl font-black text-primary italic">
              {leads.filter((l: any) => l.deployment_preference === 'supabase_cloud').length}
            </h3>
            <p className="text-[10px] font-bold text-primary mb-1.5">Supabase Cloud</p>
          </div>
        </div>
        <div className="glass-card !p-6 border-amber-400/10 bg-surface/30">
          <p className="text-text-muted text-[10px] font-black uppercase tracking-[0.2em] mb-1">Self-Hosted Pref</p>
          <div className="flex items-end gap-3">
            <h3 className="text-3xl font-black text-amber-400 italic">
              {leads.filter((l: any) => l.deployment_preference === 'self_hosted_supabase').length}
            </h3>
            <p className="text-[10px] font-bold text-amber-400 mb-1.5">Self-Hosted</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {leads.map((lead: any) => {
          const dep = DEPLOYMENT_CONFIG[lead.deployment_preference] ?? DEPLOYMENT_CONFIG.not_sure;
          return (
              ...
              <div className="p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-6 bg-black/20 rounded-2xl p-6 border border-white/5 relative">
                  ...
                  <p className="text-sm text-text-muted leading-relaxed italic">
                    &ldquo;{lead.message || 'No additional project intelligence provided.'}&rdquo;
                  </p>
                </div>

                {/* Status & Deployment */}
                <div className="lg:col-span-3 flex flex-col h-full justify-between items-end text-right gap-3">
                  <div className="flex flex-col items-end gap-3">
                    ...
                    {/* Deployment badge */}
                    <span className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full border ${dep.cls}`}>
                      {dep.icon}
                      {dep.label}
                    </span>
